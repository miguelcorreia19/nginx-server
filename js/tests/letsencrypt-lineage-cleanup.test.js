...
describe('letsencrypt handler — zero configured entries, an orphaned lineage exists', () => {
  ...
  it('generates no site configuration, fragments or redirects', async () => {
    ...
    // Nothing at all now: the backup directory used to be created here
    // unconditionally, even on a path that backs nothing up.
    expect(commandSafe.mock.calls.map(([bin]) => bin)).toEqual([]);
  });
