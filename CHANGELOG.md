...
- **Proxying to Docker services now uses dynamically resolved upstreams.** A site that proxied to another container with a literal `proxy_pass http://service:port` had that name resolved once, at configuration load, so a backend recreated under a new IP kept receiving connections at the old address (`502 Bad Gateway`) until nginx was reloaded or restarted. Every proxying example (`examples/custom-certs`, `examples/custom-configs`, `examples/dev`, `examples/cloudflare-custom-certs`) and the documented site configs now proxy through a named `upstream` with `zone`, `resolver 127.0.0.11 valid=1s` and `server <service>:<port> resolve`, which re-resolves the name through Docker's embedded DNS at runtime. Verified against nginx 1.31.4, including that nginx now starts while the backend does not exist yet. The validity is one second on purpose: Docker can hand a freed address to another container, and nginx keeps sending requests to the old address until the cached answer expires — measured at about one second with `valid=1s`, against about ten with the `10s` first considered. Upstream and zone names are scoped to the site as well as the backend (`<site>_<service>_upstream`), because every site file shares one `http` context and two sites proxying the same service under one name are refused as `duplicate upstream`. Existing site files with a literal `proxy_pass` are accepted exactly as before; they gain the fix only once migrated. See `docs/configuration.md` → Proxying to other Docker containers, and the new troubleshooting entry for the stale-IP symptom and its temporary reload workaround.

### Added

- `tests/integration/dynamic-upstream-dns.sh`, a Docker integration test (also a CI job) that builds the image and proves the re-resolution end to end: nginx starts with the backend absent, the backend is recreated on a different IP while another container takes over its old address, and traffic converges on the replacement within a bound sized for the one-second validity — and stays there — with no nginx reload or restart. It also checks that two sites proxying the same service are refused under one upstream name and accepted under site-scoped names.
...
- Added a second internal filesystem transaction, the counterpart to the one below: it installs a certificate from a backup into a name that has no local certificate at all, for a container starting with an empty Certbot directory. It refuses any name that is not completely empty, installs the certificate's material before the file that makes it visible, and has its own interrupted-transaction recovery in its own namespace, so the two transactions cannot be confused for one another. **Nothing calls it yet and no behaviour changes** — first-boot loading from a backup is still handled by the existing mechanism.
