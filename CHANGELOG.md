...
### Added — optional Fail2ban support
- New **opt-in** Fail2ban layer, disabled by default and gated entirely on `FAIL2BAN_ENABLED` (`true` enables; unset/`false` disable). When disabled, behavior is unchanged — no new processes, no config written, healthcheck untouched.
- Two error-log-based jails enabled when on: `nginx-http-auth` (auth brute-force) and `nginx-botsearch` (script/exploit probing). Both read nginx's error log, whose format is fixed by nginx and unaffected by this image's custom access-log format — no custom regex or dedicated log needed.
- Backend `polling` (Alpine has no pyinotify); ban action `iptables-multiport`; Fail2ban logs to stdout (visible via `docker logs`).
- New tunables `FAIL2BAN_BANTIME` (3600), `FAIL2BAN_FINDTIME` (3600), `FAIL2BAN_MAXRETRY` (6), `FAIL2BAN_IGNOREIP` (`127.0.0.1/8 ::1`), validated in `validate.js`; invalid values warn and fall back to the default rather than aborting startup.
- Config generated by a new `js/fail2ban` module (mirrors the existing mode-handler/template pattern) and launched by a new `fail2ban.sh` background helper (mirrors `reload.sh`). nginx stays the foreground process; Fail2ban startup failures are non-fatal.
- Alpine startup fixes: pre-create `/var/run/fail2ban` and `/var/lib/fail2ban`, and remove the package's `jail.d/alpine-ssh.conf` (which would otherwise abort startup in this SSH-less image).
- Requires the `NET_ADMIN` capability for bans to take effect; missing capability is detected, warned about, and skipped gracefully (nginx continues).
- Docs: new "Fail2ban (optional)" README section covering enabling, `NET_ADMIN`, reverse-proxy/real-IP caveats, and `FAIL2BAN_IGNOREIP`.
- Tests: Fail2ban config-generation/gating/fallback, `validatePositiveInt`/`validateIgnoreIp` validators, and Dockerfile/shell assertions; `fail2ban.sh` added to the `bash -n` checks.
