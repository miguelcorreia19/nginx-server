...
- **`reload.sh`** — watches the mounted `sites/` and custom-config directories with `inotifywait` (non-recursively, filtered to entries ending in `.conf`) and runs `nginx -s reload` when one of them is written, created, renamed in or out, deleted, or replaced by a symlink swap — logging success/failure. Runs for the container's lifetime as a background helper. Whether a *host-side* edit reaches it depends on the bind mount propagating inotify events, which Docker Desktop may not do; see [Troubleshooting](troubleshooting.md#nginx-not-reloading-after-config-change).
  It also subscribes to `delete_self`/`move_self`/`unmount` on the two directories themselves. inotify follows the inode, so a watched directory that is removed, renamed or remounted leaves its watch attached to something that no longer exists while the path appears fine — coverage that is silently half gone. The watcher treats that as fatal and exits non-zero rather than running on with one directory covered, which surfaces through the [healthcheck](#healthcheck-architecture) like any other dead helper. It does not re-create the directory or re-register the watch; see [Troubleshooting](troubleshooting.md#replacing-a-watched-directory-stops-the-watcher).
...
- **Least privilege for optional features.** The `NET_ADMIN` capability is required only when `FAIL2BAN_ENABLED=true` (so Fail2ban can install iptables rules). Only the Fail2ban compose example (`examples/fail2ban/`) grants it; the other examples do not, and it should be added only when Fail2ban is enabled.
