...
- **`entrypoint.js`** — the config-generation driver. It defaults `ENVIRONMENT` to `production`, validates `config.json` (must be a JSON object; every entry runs through `validateConfigEntry`), then dispatches on `ENVIRONMENT`:
  ...
  - `production` / `prod` → a filesystem preflight (`js/preflight.js`) checks every entry's local files — its site config, and a `custom` entry's certificate files — *before* running the `letsencrypt`, `custom`, and `http` handlers in sequence; a missing required file is fatal, for every entry, before any handler runs.
...
- **JS-based generation.** Each runtime mode is a handler module under `js/` (`letsencrypt/`, `dev/`, `custom/`, `http/`), each exporting a single async function and rendering config from files under its `templates/` directory. Shared helpers live in `js/utils.js`; schema validation lives in `js/validate.js`; production filesystem preflight lives in `js/preflight.js`.
- **Validation flow.** `config.json` is validated up front (structure + every entry via `validateConfigEntry`) before any handler runs, so failures are reported clearly instead of producing broken nginx config. In production, every entry's required local files are then preflighted (`js/preflight.js`) before any handler runs — a missing site config or `custom` certificate file is fatal, not a silent skip. After generation, the *assembled* config is validated with `nginx -t` (`validateNginxConfig`); a failure here aborts the container with an actionable message rather than letting nginx crash-loop.
...
- **`js/`** — `entrypoint.js` (the startup driver), one handler package per mode (`letsencrypt/`, `dev/`, `custom/`, `http/`, `fail2ban/`) each with an `index.js` and `templates/`, shared `utils.js` / `validate.js` / `preflight.js`, and the `tests/` suite. The `letsencrypt/` package also holds `certbot_renew.js` and `manage_certs.js`.
