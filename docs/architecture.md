...
- **Where a shell is still used, and why.** `command()` is reserved for commands that are either fixed strings (`certbot certificates`, `crond`, and the `crontab -l | grep -v … | crontab -` pipeline, which genuinely needs shell piping) or that depend on shell redirection: the two `openssl` self-signed-certificate invocations end in `2>&1`, which is what makes `command()` treat openssl's stderr progress output as success. The only value interpolated into any of them is the certificate ID in `js/letsencrypt/manage_certs.js`, and `validateCertId()` has already restricted it to letters, digits, dots, hyphens and underscores at startup — so there is no shell metacharacter left to inject.
