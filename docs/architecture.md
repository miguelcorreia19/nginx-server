...
- **`entrypoint.js`** — the config-generation driver. It defaults `ENVIRONMENT` to `production`, validates `config.json` (must be a JSON object; every entry runs through `validateConfigEntry`), then dispatches on `ENVIRONMENT`:
  - `development` / `dev` → a filesystem preflight (`js/preflight.js`) requires `/home/nginx/sites/dev.conf` to exist *before* running the `dev` handler (self-signed certs); a missing `dev.conf` is fatal.
...
**Production startup rebuilds generated nginx `conf.d` state from the current configuration, so a restart of the same container does not retain active site configuration from previous startups.**
...
- **Production startup owns both directories.** After preflight and before any handler, `reconcileGeneratedConfig()` (`js/reconcile.js`) clears them and restores the default `:80` and `:443` vhosts as the baseline. This runs on every production startup, whatever any individual mode's entry count is.
- **Mode handlers are additive.** `letsencrypt`, `custom` and `http` only add the sites, redirects and links their own mode currently has configured. No handler cleans up after another, and no handler restores a default vhost.
- **Why.** Previously the only broad cleanup lived inside the Let's Encrypt handler and ran only when it had at least one entry of its own. A same-container restart could therefore keep serving sites that had been removed from `config.json`, keep an obsolete HTTP→HTTPS redirect after `http_redirect` was set to `false`, or leave a site linked from both port directories after a mode change. A deleted site file could also leave a dangling link that failed `nginx -t` and prevented the container from starting.
- **Failure is fatal.** If the reset or either default-vhost restore fails, startup aborts before any handler runs; nginx is never started from a partially reset tree.
- **Development is excluded.** `js/dev/index.js` deliberately removes both default vhosts because its own generated fragment declares the HTTPS `default_server`. A later *production* startup restores them, which is what reconciles a development→production switch on the same container.
