...
- **Where a shell is still used, and why.** `command()` is reserved for commands that are either fixed strings (`certbot certificates`, `crond`, and the `crontab -l | grep -v … | crontab -` pipeline, which genuinely needs shell piping) or that carry an interpolated value: the two `openssl` self-signed-certificate invocations. Both `command()` and `commandSafe()` decide success or failure from the child process's own exit status, never from which stream carried output, so a process that exits 0 after writing only to stderr — openssl's progress dots included — resolves rather than being rejected. The two `openssl` invocations still end in `2>&1`, but that redirect is no longer load-bearing for this decision; it remains in place as a leftover from the prior contract, pending a separate cleanup. The only value interpolated into either of them is the certificate ID in `js/letsencrypt/manage_certs.js`, and `validateCertId()` has already restricted it to letters, digits, dots, hyphens and underscores at startup — so there is no shell metacharacter left to inject.
...
- **Certificates never exposed via the shell.** The Node layer never reads certificate or private-key *contents* — it only copies files and logs paths — so no key material can pass through a shell or reach a log. The paths Certbot reports are passed as array arguments to `execFile`, never interpolated into shell strings; the one exception is the self-signed fallback in `js/letsencrypt/manage_certs.js`, whose `openssl` command names its own output paths in a shell string built from the validated certificate ID.
